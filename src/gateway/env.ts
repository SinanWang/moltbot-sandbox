...
export function buildEnvVars(env: MoltbotEnv): Record<string, string> {
  ...
  const rawBaseUrl = env.AI_GATEWAY_BASE_URL || env.DEEPSEEK_BASE_URL || env.OPENROUTER_BASE_URL || env.OPENAI_BASE_URL || env.ANTHROPIC_BASE_URL;
  ...
  const isDeepSeek = normalizedBaseUrl?.includes('deepseek.com');
  ...
  if (!envVars.OPENAI_BASE_URL && env.DEEPSEEK_API_KEY) {
    envVars.OPENAI_BASE_URL = 'https://api.deepseek.com';
  }
